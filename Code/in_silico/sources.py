...
def concentration_delta(params: np.ndarray, r: Union[np.ndarray, float], t: Union[np.array, float]) -> Union[np.ndarray, float]:
    """
    ...
    if not isinstance(r, (Number, np.ndarray)):
        raise TypeError('r must be either a number or a numpy array, but it is {}'.format(type(r)))
    Crt = M/np.sqrt(4*np.pi*D*t) * np.exp(-r**2/(4*D*t))
    return Crt
