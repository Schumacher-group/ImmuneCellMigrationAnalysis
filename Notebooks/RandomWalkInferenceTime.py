...
def time_slice(space):
    ...
    t10 = space[(space['t'] >=  300)  & (space['t'] <=900)]
    t20 = space[(space['t'] >= 600)  & (space['t'] <= 1800)]
    t30 = space[(space['t'] >= 900)  & (space['t'] <= 2700)]
    t40 = space[(space['t'] >= 1200)  & (space['t'] <= 3600)]
    t50 = space[(space['t'] >= 1500)  & (space['t'] <= 4500)]
    times = [t5,t10,t20,t30,t40,t50]
    return times
...
for i in range(len(s_distance)):
    for j in range(len(times)):
        ...
        np.save('../data/np_array/WB Time WT-{}{}'.format(,i,j),inf_out)
